...
namespace AdventOfCode2019
{
	internal class Program
	{
		...
		private static void Day11()
		{
				...
				new Intcode.Engine()
					...
					.OnOutput(engine =>
					{
						...
						else
						{
							...
							var panelColor = map[x][y] == 'W' ? ColorWhite : ColorBlack;
			...
			var map1 = PaintHull(ColorBlack);
			var numberOfPoints = map1.AllPoints.Count();
			Console.WriteLine($"Day 11 Puzzle 1: {numberOfPoints}");
			Debug.Assert(numberOfPoints == 2343);

			var map2 = PaintHull(ColorWhite);
			var points = map2.AllPoints.ToList();
			var xMin = points.Min(z => z.X);
			var xMax = points.Max(z => z.X);
			var yMin = points.Min(z => z.Y);
			var yMax = points.Max(z => z.Y);
			const char blackBoxChar = '\u2588';
			var lines = Enumerable.Range(yMin, yMax - yMin + 1)
				.Select(yyy => Enumerable.Range(xMin, xMax - xMin + 1)
					.Select(xxx => map2[xxx][yyy] == 'W' ? blackBoxChar : ' ')
					.ToArray()
				)
