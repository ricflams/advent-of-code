...
namespace AdventOfCode2019
{
	internal class Program
	{
		...
		private static void Day5()
		{
			...
			var engine = new Intcode.Engine();
			engine.Initialize(mem);
			engine.Data.Input.Add(1);
			engine.Execute();
			var output1 = string.Join(" ", engine.Data.Output);
			Console.WriteLine($"Day5: Puzzle1: {output1}");

			engine = new Intcode.Engine();
			engine.Initialize(mem);
			engine.Data.Input.Add(5);
			engine.Execute();
			var output2 = string.Join(" ", engine.Data.Output);
			Console.WriteLine($"Day5: Puzzle2: {output2}");
		}
	}
}
