...
namespace AdventOfCode.Y2022.Day18.Raw
{
	internal class Puzzle : Puzzle<long, long>
	{
		...
		{
			var cubes = input.Select(s => s.ToIntArray()).Select(c => new Point3D(c[0], c[1], c[2])).ToList();
		...
		private int CountSurfaces(List<Point3D> world)
		{
			var sides = world.Count * 6;
			for (var i = 0; i < world.Count; i++)
			{
				for (var j = i+1; j < world.Count; j++)
				{
					var (c1, c2) = (world[i], world[j]);
					var dx = Math.Abs(c1.X - c2.X);
					var dy = Math.Abs(c1.Y - c2.Y);
					var dz = Math.Abs(c1.Z - c2.Z);
					if (dx+dy+dz == 1)
		...
		protected override long Part2(string[] input)
		{
			...
			var world = new HashSet<Point3D>(cubes);
			var holes = new HashSet<Point3D>();
			...
			var displacements = new[] { new Point3D(-1,0,0), new Point3D(1,0,0), new Point3D(0,-1,0), new Point3D(0,1,0), new Point3D(0,0,-1), new Point3D(0,0,1) };

			var surface = CountSurfaces(cubes);

			var pp = holes.TryGetValue(new Point3D(2,2,5), out var xx);

			var outer = ExtractCohesiveChunk(holes, new Point3D(min.X-1, min.Y-1, min.Z-1));

			while (holes.Any())
			{
				var cavity = ExtractCohesiveChunk(holes, holes.First());
				surface -= CountSurfaces(cavity.ToList());
			}

			return surface;

			HashSet<Point3D> ExtractCohesiveChunk(HashSet<Point3D> world, Point3D start)
			{
				...
				while (removals.TryDequeue(out var p))
				{
					...
					chunk.Add(p);
					world.Remove(p);
					foreach (var n in displacements.Select(d => p.Add(d)).Where(c => !chunk.Contains(c)).Where(c => world.Contains(c)).ToArray())
					{
						// if (n == new Point3D(2, 2, 5))
						// 	;
						//Extract(n);
						removals.Enqueue(n);
					}
