...
namespace AdventOfCode.Y2021.Day22
{
	internal class Puzzle : Puzzle<long, long>
	{
		...
		internal class Cube
		{
			...
			public override string ToString() => $"size={Size} x={Bot.X}..{Top.X} y={Bot.Y}..{Top.Y} z={Bot.Z}..{Top.Z}";
			...
			{
				overlap =
					other.Corners.Any(p => Contains(p)) ? Overlap(other) :
					Corners.Any(p => other.Contains(p)) ? Overlap(other) :
					null;
				return overlap != null;
			}
		...
		//		for (var j = i + 1; j < cubes.Count; j++)
		//		{
		//			Debug.Assert(!cubes[i].HasOverlap(cubes[j], out var _));
		//		}
		//	}
		//}
